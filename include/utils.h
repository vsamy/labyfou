...
{
	return static_cast<CategoryType>(static_cast<enum_type>(lhs) | static_cast<enum_type>(rhs));
}
inline CategoryType operator&(CategoryType lhs, CategoryType rhs)
{
	return static_cast<CategoryType>(static_cast<enum_type>(lhs) & static_cast<enum_type>(rhs));
...
// Functions for simplifying manipulations with SFML objects
// Check if the class has the function getLocalBounds
// This function is only used in debug mode
template<typename T>
bool checkClass(T /* obj */)
{
	return std::is_base_of<sf::Sprite, T>::value |
		std::is_base_of<sf::Shape, T>::value |
		std::is_base_of<sf::Text, T>::value;
}

template<typename T>
void centerOrigin(T& obj)
{
	assert(checkClass(obj));
	sf::FloatRect bounds = obj.getLocalBounds();
	obj.setOrigin(bounds.width / 2.f, bounds.height / 2.f);
}


// Map sf::Keyboard::Key to a String name
inline std::string toKeyboardName(sf::Keyboard::Key key)
{
